...
class Gayabelajar extends CI_Controller
{
    ...
    public function edit($id_gaya_belajar = NULL)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['gaya_belajar'] = $this->db->get('gaya_belajar', ['id_gaya_belajar' => $id_gaya_belajar])->result();
            $this->load->view('admin/index', $data);
        } else {
            ...
            $this->db->update($data, ['id_gaya_belajar' => $id_gaya_belajar], 'gaya_belajar');
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Gaya belajar baru telah diedit!</div>');
            redirect('admin/gayabelajar');
        }
    }
}
